...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a LYBERIXV3 address (e.g. %1)").arg("2Qb1kNGfyJLi37bagDQswn2EkGjSCBzokD"));
#endif
...
{
    // return if URI is not valid or is no LYBERIXV3: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::LYBERIXV3, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert lyberixv3:// to lyberixv3:
    //
    //    Cannot handle this later, because lyberixv3:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LYBERIXV3, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open lyberixv3.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LYBERIXV3.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for LYBERIXV3.lnk
...
{
    return GetAutostartDir() / "lyberixv3.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a lyberixv3.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LYBERIXV3\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lyberixv3 app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add lyberixv3 app to startup item list
